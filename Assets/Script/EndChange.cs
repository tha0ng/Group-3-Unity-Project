...
using TMPro; //access unity text mesh pro system
using UnityEngine.SceneManagement; //access unity scene managememt

public class EndChange : MonoBehaviour
{
    public TextMeshProUGUI textOB; //text to display on screen
    public string description = "Description."; //input description to tell us what to say when we interact on the object
    ...
    void Update() //if we're in reach or we're looking at the object
    {
        ...
        {
            Debug.Log("loading cutscene"); // check if entering cutscene works
            SceneManager.LoadScene("Ending"); //change the scene
        }   
